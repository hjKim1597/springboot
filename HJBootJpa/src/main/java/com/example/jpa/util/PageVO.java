...
public class PageVO<T> {
    ...
    public PageVO(Page<T> pageable) {
        this.page = pageable.getNumber() + 1; //현재 페이지의 번호를 가져옴/ 페이지는 0부터 시작이지만 사용자는 1부터 시작이 일반적
        this.amount = pageable.getSize(); //한 페이지에 표시될 데이터의 개수
        this.total = pageable.getTotalElements(); // 전체 데이터의 총 개수
        this.pageData = pageable.getContent(); // 현재 페이지에서 조회된 실제 데이터를 가져옴
