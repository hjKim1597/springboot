...
//        System.out.println( t.get(memo.writer) );
    ...
    public List<Memo> dslJoin() {
        ...
        List<Memo> list = jpaQueryFactory.select(memo)
                .from(memo)
                //.join(memo.member, member) //조인의 조건 : 엔티티A.엔티티B = 엔티티B
    ...
    public Page<MemberMemoDTO> dslJoinPaging(Criteria cri, Pageable pageable) {
        ...
        if(cri.getSearchType().equals("text") ){
            builder.and((memo.text.like( "%"+ cri.getSearchName() +"%" ))); //where text l ike %value%
        }
        if(cri.getSearchType().equals("writer") ){
            builder.and((memo.writer.like( "%"+ cri.getSearchName() +"%" ))); //where writer l ike %value%
        }
        if(cri.getSearchType().equals("textWriter") ){
            builder.and((memo.text.like( "%"+ cri.getSearchName() +"%" ))); //where text l ike x or writer x
        ...
        List<MemberMemoDTO> list = jpaQueryFactory.select(
                ...
                .offset(pageable.getOffset() ) // limit 0, 10 이중에 앞의 값을 의미함
                .limit(pageable.getPageSize() ) // limit 0, 10 이중에 뒤의 값을 의미함
                .fetch();
        ...
        //4. Page객체에 맵핑
        PageImpl<MemberMemoDTO> page = new PageImpl<>(list, pageable, total); //데이터, 페이지객체, 전체 게시글 수
